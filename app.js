 ...
 * Depends on: backend.js (DB, Auth, Users, Applications, Complaints, OTP, SERVICES, Session)
 */
...
let selectedService = null;   // { name, dept } set when opening apply modal
...
/** Show/hide alert banners inside forms */
...
function doRegister() {
  ...
  else if (Users.usernameExists(username)) { showErr('err-username', 'Username already taken');           valid = false; }
  if (!/^\d{12}$/.test(aadhar))        { showErr('err-aadhar',   'Aadhar must be exactly 12 digits');    valid = false; }
  else if (Users.aadharExists(aadhar)) { showErr('err-aadhar',   'Aadhar already registered');            valid = false; }
  ...
  // Second click → verify OTP and save user
  const digits     = document.querySelectorAll('.otp-digit');
...
function doLogin() {
  ...
  const result = Auth.login(loginId, pwd, role);
...
function updateStats() {
  ...
  if (currentUser.role === 'citizen') {
    ...
    const mine = Applications.getByUser(currentUser.id);
    ...
    document.getElementById('cit-complaints').textContent = Complaints.getByUser(currentUser.id).length;
    ...
    document.getElementById('adm-citizens').textContent = Users.getAll().filter(u => u.role === 'citizen').length;
...
/* ══════════════════════════════════════════
   ...
   ══════════════════════════════════════════ */
function loadMyApps() {
  const apps = Applications.getByUser(currentUser.id);
...
function loadComplaints() {
  const comps = Complaints.getByUser(currentUser.id);
...
function submitComplaint() {
  ...
  Complaints.create({ userId: currentUser.id, dept, category: cat, subject, desc });
...
/* ══════════════════════════════════════════
   ...
   ══════════════════════════════════════════ */
function loadAllApps(filter) {
  const apps = filter ? Applications.getByStatus(filter) : Applications.getAll();
...
/* ══════════════════════════════════════════
   ...
   ══════════════════════════════════════════ */
function loadDeptApps() {
  const apps = Applications.getByDept(currentUser.dept);
...
function updateStatus() {
  ...
  const updated   = Applications.updateStatus(selectedAppId, newStatus, remark);
  if (!updated) return;
...
function loadDeptComplaints() {
  const comps = currentUser.role === 'admin'
    ? Complaints.getAll()
    : Complaints.getByDept(currentUser.dept);
...
function resolveComplaint(id) {
  const resolved = Complaints.resolve(id);
  if (resolved) {
    loadDeptComplaints();
...
function loadAssignments() {
  const pending   = Applications.getByStatus('pending');
  const deptHeads = Users.getAll().filter(u => u.role === 'department_head');
